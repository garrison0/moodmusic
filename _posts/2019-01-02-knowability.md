...
i'll let the thesis introduce what i take to be the central tenant of epistemicism, margin for error principles: 
...
using the topological semantics, i define what it means for a predicate $P$ to be vague at a world $U$ as such: 
...
>
>$$\begin{align*}
(K(H(n)) \rightarrow H(n-1) &\implies (KK(Hn) \rightarrow K(H(n-1))  &  (distribution) \\
										 &\implies K(H(n-1)) 								   & (KK) \\ 
